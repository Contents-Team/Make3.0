...
>
> v1: 단일 문자 prefix + NN 순번 + 영문 강제. v2: 3-Layer Identity 도입했으나 prefix 선택화로 자가설명 손실. v3: 자가설명 강제 (mandatory prefix + namespace + suffix vocab). v4: prefix 정책은 v3 유지 + **Tier 책임 분리** — 사용자는 Tier A 만 명명, 시스템이 Tier C 자동 파생. v5: **소스 에셋 파일시스템 레이어** 추가 — `.make` 내부 식별자(v4)와 외부 원본 파일(FBX/PNG/WAV/MP4) 명명을 명시적으로 분리, `v` marker 로 Source vs Instance 구분.

## Two-Layer Naming Model (v5)

| Layer | 적용 대상 | 형식 | 정의 위치 |
|---|---|---|---|
| **L1 — Source Asset (filesystem)** | 모델러/아티스트가 납품하는 원본 파일 (`.fbx`, `.png`, `.wav`, `.mp4`, `.ttf`) | **PascalCase** + `_v<NN>` marker + 확장자 | 이 문서 "Source Asset Filesystem Naming" |
| **L2 — `.make` Internal Identifier** | `.make` 임포트 이후 glTF node/material/texture/animation 이름 | **lower_snake_case** + 3-letter kind prefix | 이 문서 "Identifier — `.make` Internal" (구 v4 본문) |

**왜 두 형식이 다른가**:
- L1 (Source) 은 모델러 도구(Blender/Maya/Photoshop) 와 OS 파일 탐색기에서 다뤄짐 — PascalCase 가 산업 관례이며 사람 가독성 우선. `vNN` marker 로 버전 관리.
- L2 (Internal) 은 코드·JSON·grep 컨텍스트에서 다뤄짐 — `lower_snake_case` + prefix 가 에이전트 파싱·정렬에 유리. 순번 없는 stable id 가 필요.
- 임포트 파이프라인이 **L1 → L2 정규화** 를 자동 수행 (예: `GlassBottle_v01.fbx` → `mdl_glass_bottle`).

### L1 → L2 Normalization (Import-time Mirror)

```
Source file:        GlassBottle_Cap_v01.fbx           (L1, PascalCase + vNN + ext)
                         │
                         ▼ (1) 확장자 → kind prefix 매핑
                         ▼ (2) v<NN> marker 제거 (버전은 ResourceID 로 추적)
                         ▼ (3) PascalCase → lower_snake_case
                         ▼ (4) 부모-자식 `_` → `__` 승격
Internal name:      mdl_glass_bottle__cap              (L2, snake_case + prefix)
```

L1 의 `_State_` 토큰은 L2 의 `__<state>` suffix 로 변환: `GlassBottle_Open_v01.fbx` → `mdl_glass_bottle__open`.

## Source Asset Filesystem Naming (L1, v5)
...
| Version `v` + 2자리 | `_v01`, `_v02` | `_V1`, `_ver01` |
| 중복 타입 토큰 금지 | `GlassBottle_v01.fbx` | `GlassBottle_MOD_v01.fbx` |
| 영문 전용 | `BatteryPack_v01` | `배터리팩_v01` |

> ⚠️ Version 히스토리 (`_v01`, `_v02`) 는 납품 전 WIP 관리용. 최종 납품 에셋은 별도 합의 없으면 단일 깨끗한 버전만 유지.
...
확장자가 타입 선언. **타입 토큰 불필요.** 카테고리는 `Name` 토큰 내부 prefix 로 임베드.
...
> 카테고리 prefix 와 description 사이에 구분자 없음 — 둘 다 단일 `Name` 토큰의 일부.
...
확장자가 타입 선언. 컨텍스트는 `Name` 토큰 내부 prefix.
...
INTROMain_01.mp4
OUTROCredit_01.mp4
CUTAssemblyStep01_01.mp4
TUTBatterySwapStep01_01.mp4
TUTBatterySwapStep02_01.mp4
LOOPWorkshopBackground_01.mp4
```
...
## Identifier — `.make` Internal (L2, v4 본문)

> 이하 v4 본문은 `.make` 내부 식별자(L2) 에 적용. L1 소스 파일은 위 "Source Asset Filesystem Naming" 참조.
...
## Validation Checklist (`.make` 익스포트 전, v5)

### L1 — Source Asset Filesystem 검증 (v5 신규)
- [ ] 파일명에 공백 없음, 구분자는 `_` 만
- [ ] 토큰별 PascalCase (`GlassBottle`, `BatteryPack`), kebab/snake/소문자 시작 없음
- [ ] Variant `_NN` 은 2자리 고정 (`_01`, `_02`), `_1`/`_001` 금지
- [ ] Version marker `_v<NN>` 는 소스 파일에만, 인스턴스 표기에는 금지
- [ ] 확장자가 타입을 선언하는 경우 이름에 중복 타입 토큰 없음 (`_MOD_`, `_TEX_` 등 금지)
- [ ] PNG 는 Role 토큰 (`Sprite`/`ALB`/`NRM`/`RGH`/`MET`/`EMI`/`MSK`/`AO`) 필수, `v<NN>` 미사용
- [ ] Audio 카테고리 prefix (`BGM`/`SFX`/`VO`/`AMB`) 가 `Name` 토큰 내부에 임베드
- [ ] Video 컨텍스트 prefix (`CUT`/`TUT`/`LOOP`/`INTRO`/`OUTRO`) 임베드
